    ...
    <>
      ...
      <div className="amz-page">
        ...
        <div className="amz-pdp">
          <Gallery main={data.images.main} thumbnails={data.images.thumbnails} title={data.title} />

          <div className="col-center">
            <TitleBlock title={data.title} brandName={data.brand.name} brandUrl={data.brand.url} rating={data.rating} />

            <hr className="amz-divider" />
            <div className="amz-price-inline"><span className="symbol">$</span>{displayPrice.toFixed(2)}</div>

            {dimension && (
              <div className="amz-variant-row">
                <span className="a-size-base a-color-secondary amz-variant-row__label">
                  {dimension.dimension}: {selectedOption?.value}
                </span>
                  ...
                  >
                    {dimension.options.map((opt) => (
                      <li
                        ...
                        className={`a-button${opt.value === selectedValue ? ' a-button-selected is-selected' : ''}`}
                      >
                        <button type="button" tabIndex={-1} onClick={() => setSelectedValue(opt.value)}>
                          ...
                          {opt.availability && (
                            <span id="twisterAvailability" className="a-size-mini"> {opt.availability}</span>
                          )}
                        </button>
                      </li>
                    ))}
          ...
          <div className="col-buybox amz-buybox" id="buybox">
            <div id="corePriceDisplay_desktop_feature_div">
              <span className="a-price aok-align-center reinventPricePriceToPayMargin priceToPay price-current" data-a-size="xl" data-a-color="base">
              ...
              {data.price.list != null && (
                <span className="a-price a-text-price price-strike" data-a-strike="true">
                  <span className="a-offscreen">${data.price.list.toFixed(2)}</span>
                  <span aria-hidden="true">List: ${data.price.list.toFixed(2)}</span>
                </span>
              )}
            </div>

            {data.coupon && (
              <div id="couponsInBuybox_feature_div" className="promoPriceBlockMessage amz-coupon">☑ {data.coupon}</div>
            )}
            {data.promo && (
            ...
            <div id="deliveryBlockMessage" className="amz-delivery">
              <span data-csa-c-delivery-time={data.delivery.estimate}>Delivery <b>{data.delivery.estimate}</b></span>
            </div>
            ...
            <div className="a-box-inner" style={{ marginTop: 12 }}>
              <input type="submit" id="add-to-cart-button" name="submit.add-to-cart" className="a-button-input btn btn-cart" value="Add to Cart" readOnly />
              <input type="submit" id="buy-now-button" name="submit.buy-now" className="a-button-input btn btn-buy" value="Buy Now" readOnly />
            </div>
